...
public interface WorldRecordRepository extends JpaRepository<Points, Integer> {
    ...
    @Query("SELECT new com.typerf1.typerf1.dto.points.Record(p.name, p.surname, gp.name, s.year, MAX(pt.number)) " +
            "FROM Points pt " +
            "JOIN pt.participant p " +
            "JOIN pt.session ses " +
            "JOIN ses.grandPrix gp " +
    ...
    @Query("SELECT new com.typerf1.typerf1.dto.points.Record(p.name, p.surname, gp.name, s.year, MIN(pt.number)) " +
            "FROM Points pt " +
            "JOIN pt.participant p " +
            "JOIN pt.session ses " +
            "JOIN ses.grandPrix gp " +
